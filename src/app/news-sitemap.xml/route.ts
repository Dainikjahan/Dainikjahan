...
export async function GET() {
  ...
  const items = articles.map((article) => {
    ...
    return `<url><loc>${siteUrl}/article/${encodeURIComponent(article.slug)}</loc><news:news><news:publication><news:name>দৈনিক জাহান</news:name><news:language>bn</news:language></news:publication><news:publication_date>${published}</news:publication_date><news:title>${escapeXml(article.title)}</news:title></news:news>${image}</url>`;
  }).join("");
